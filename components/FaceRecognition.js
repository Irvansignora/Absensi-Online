...
// ─── MediaPipe loader pakai dynamic import (ES module) ───────────────────────
// Tidak pakai script tag — pakai import() langsung ke .mjs supaya tidak 404
...
async function loadMediaPipe() {
  ...
  // Dynamic import ke ES module — ini yang benar, bukan vision_bundle.js
...
// ─── State ───────────────────────────────────────────────────────────────────
const S = { LOADING: 'loading', DETECTING: 'detecting', VERIFIED: 'verified', FAILED: 'failed' }
...
export default function FaceRecognition({ onVerified, onSkip, mode = 'checkin' }) {
  ...
  function startLoop(detector) {
    ...
    async function detect() {
      ...
      if (isGood) {
        stableFrames++
        setMessage(`Memverifikasi wajah... ${Math.round((stableFrames / NEEDED) * 100)}%`)
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center p-4"
      ...
      <div className="relative w-full max-w-sm aspect-[3/4] rounded-3xl overflow-hidden bg-slate-900 border-2 border-white/10"
        style={{ boxShadow: isVerified ? '0 0 40px rgba(16,185,129,0.4)' : '0 0 40px rgba(37,99,235,0.2)' }}>

        <video ref={videoRef} muted playsInline
          className="absolute inset-0 w-full h-full object-cover"
          style={{ transform: 'scaleX(-1)', display: isLoading || isFailed ? 'none' : 'block' }} />
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-6 p-8">
            ...
            <div className="w-full">
              <div className="flex justify-between text-xs text-slate-400 mb-2">
                <span>Memuat MediaPipe...</span><span>{progress}%</span>
              </div>
      ...
      {state === S.DETECTING && (
        <div className="mt-4 grid grid-cols-3 gap-2 w-full max-w-sm">
          {[{ icon: '💡', tip: 'Cahaya terang' }, { icon: '👤', tip: 'Tanpa masker' }, { icon: '📐', tip: 'Wajah di tengah' }].map(t => (
